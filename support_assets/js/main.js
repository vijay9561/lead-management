$("#lead_submiting_forms").validate({
    ...
    submitHandler: function (form) {
         var formData = new FormData($("#lead_submiting_forms")[0]); 
        $.ajax({
            type: "POST",
            url: BASE_URL + 'submit_leadwise_status',
            data:formData,
            processData: false,
            contentType: false,
            beforeSend: function () {
...
function status_changed_values(id,type){
   $("#lead_id").val(id);
   $("#status_id").val(type);
}

function lead_comment_showing(id){
  $.ajax({
    url:BASE_URL+"view_get_lead_details",
    type: "POST",
    data:{id:id},
    success: function (data) {
        $('#lead_comment_details').hide().html(data).fadeIn('slow');   
    }
  });
}
